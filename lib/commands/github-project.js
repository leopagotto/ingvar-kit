...
async function setupGitHubProject(options = {}) {
  ...
  if (!isOrgRepo) {
    ...
    if (setupPAT) {
      ...
      const tokenUrl = 'https://github.com/settings/tokens/new?description=INGVAR+Kit+Project+Access&scopes=project';
...
`;
      ...
      workflowSpinner.warn('Workflow file not found (may need to run ingvar init first)');
    }
  } catch (error) {
...
# Generated by ingvar Workflow Kit
...
`;
  ...
  console.log(chalk.cyan('     ingvar issue\n'));
...
async function testGitHubProject(options = {}) {
  ...
  if (await fs.pathExists(workflowPath)) {
    ...
    console.log(chalk.yellow('\n💡 Run ingvar init to install workflows\n'));
  }
