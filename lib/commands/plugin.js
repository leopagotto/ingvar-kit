...
class PluginCommands {
   ...
   * Usage: ingvar plugin install <package-name>
   */
  ...
  /**
   * Start a plugin
   * Usage: ingvar plugin start <name>
   */
  ...
  /**
   * Uninstall a plugin
   * Usage: ingvar plugin uninstall <package-name>
   */
...
/**
   ...
   * Usage: ingvar plugin create <name> [--template=web|basic]
   */
  ...
  static async _createBasicPlugin(name, options = {}) {
    try {
      ...
      const packageJson = {
        ...
        keywords: ['ingvar', 'plugin', 'dashboard'],
        author: 'Your Name',
        license: 'MIT',
        peerDependencies: {
          'ingvar-kit': '^5.0.0'
 ...
 * ${name} Plugin
 * Ingvar Kit Dashboard plugin template
 */
...
\`\`\`bash
ingvar plugin install leo-plugin-${name}
