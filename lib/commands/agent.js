  ...
  switch (subcommand) {
    ...
    case 'enable':
      if (!agentName) {
        ...
        console.log(chalk.gray('Usage: ingvar agent enable <agent>'));
        process.exit(1);
      }
    ...
    case 'disable':
      if (!agentName) {
        console.log(chalk.red('Error: Agent name required'));
        console.log(chalk.gray('Usage: ingvar agent disable <agent>'));
        process.exit(1);
      }
    ...
    case 'info':
      if (!agentName) {
        console.log(chalk.red('Error: Agent name required'));
        console.log(chalk.gray('Usage: ingvar agent info <agent>'));
        process.exit(1);
      }
...
async function listAgents(options) {
  ...
  console.log(chalk.gray('  ingvar agent enable <agent>   Enable a specialized agent'));
  console.log(chalk.gray('  ingvar agent disable <agent>  Disable a specialized agent'));
  console.log(chalk.gray('  ingvar agent info <agent>     Show detailed agent information'));
  console.log(chalk.gray('  ingvar agent sync             Regenerate AI instruction files\n'));
}
  ...
  try {
    ...
    if (!options.noSync) {
      ...
      if (shouldSync) {
        ...
        console.log(chalk.gray('\n💡 Run'), chalk.cyan('ingvar agent sync'), chalk.gray('later to update AI files\n'));
      }
    }
  } catch (error) {
...
async function showAgentInfo(agentName, options) {
  ...
  if (!info.alwaysEnabled) {
    if (isEnabled) {
      console.log(chalk.gray('Commands:'));
      console.log(chalk.gray(`  ingvar agent disable ${agentName}  Disable this agent`));
    } else {
      console.log(chalk.gray('Commands:'));
      console.log(chalk.gray(`  ingvar agent enable ${agentName}   Enable this agent`));
    }
    console.log();
  }
}
 ...
 */
function showHelp() {
  console.log(chalk.cyan('\n🎯 INGVAR Agent Command\n'));
  console.log(chalk.bold('Usage:'));
  console.log('  ingvar agent <subcommand> [agent] [options]\n');
  ...
  console.log(chalk.gray('  ingvar agent list'));
  console.log(chalk.gray('  ingvar agent enable frontend'));
  console.log(chalk.gray('  ingvar agent disable devops'));
  console.log(chalk.gray('  ingvar agent info backend'));
  console.log(chalk.gray('  ingvar agent sync\n'));
}
