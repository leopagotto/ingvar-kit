  ...
  try {
    switch (subcommand) {
      ...
      default:
        ...
        console.log('Run ' + chalk.cyan('ingvar config help') + ' for usage information');
        process.exit(1);
    }
  } catch (error) {
...
function handleGet(args) {
  ...
  if (!key) {
    ...
    console.log('Usage: ' + chalk.cyan('ingvar config get <key> [--global]'));
    process.exit(1);
  }
...
function handleSet(args) {
  ...
  if (!key || value === undefined) {
    ...
    console.log('Usage: ' + chalk.cyan('ingvar config set <key> <value> [--global]'));
    console.log('');
  ...
  if (key === 'auto-resolve' && value === 'false') {
    ...
    console.log('To enable auto-resolution again, run: ' + chalk.cyan('ingvar config set auto-resolve true'));
  }
}
...
function handleRemove(args) {
  ...
  if (!key) {
    console.log(chalk.red('Error: Missing configuration key'));
    console.log('Usage: ' + chalk.cyan('ingvar config remove <key> [--global]'));
    process.exit(1);
  }
...
function handleInit() {
  try {
    ...
    console.log('');
    console.log('Use ' + chalk.cyan('ingvar config set <key> <value>') + ' to customize settings');
  } catch (error) {
    if (error.message.includes('already exists')) {
      console.log(chalk.yellow('⚠') + ' Configuration file already exists');
      console.log('Use ' + chalk.cyan('ingvar config list') + ' to view current settings');
    } else {
      throw error;
    }
  }
...
function showHelp() {
  ...
  console.log('  ingvar config <command> [options]\n');
  ...
  console.log('                   When true, LEO_AUTO_INIT env var not needed\n');
  ...
  console.log('  ' + chalk.yellow('--global, -g') + '  Set/get/remove from global config (~/.ingvarrc.json)');
  ...
  console.log('  ingvar config set auto-resolve false\n');

  console.log('  ' + chalk.gray('# Enable auto-resolution globally'));
  console.log('  ingvar config set auto-resolve true --global\n');

  console.log('  ' + chalk.gray('# Check current auto-resolve setting'));
  console.log('  ingvar config get auto-resolve\n');

  console.log('  ' + chalk.gray('# List all settings with their sources'));
  console.log('  ingvar config list\n');

  console.log('  ' + chalk.gray('# Set project type'));
  console.log('  ingvar config set project-type frontend\n');
