 ...
 * This checks .ingvarrc.json first (set by ingvar init)
 */
async function getDesignSystemConfig(options = {}) {
  ...
  // Try to read from config file (set by ingvar init)
  try {
  ...
  console.log(chalk.gray('💡 Tip: Run ') + chalk.cyan('ingvar init') + chalk.gray(' to set up your project with IKEA design system\n'));
...
async function applyIngkaSkapaDesignSystem(appPath) {
  ...
  try {
    ...
    if (shouldInstallComponents) {
      ...
      console.log(chalk.gray('📦 Skipping component installation (install-components: false in .ingvarrc.json)'));
      console.log(chalk.yellow('   Install manually with:'), chalk.cyan('ingvar components install\n'));
    }
...
module.exports = async function spark(options = {}) {
  ...
  // Check if ingvar init has been run
  try {
    const configManager = require('../utils/config-manager');
    const designSystemConfig = configManager.get('design-system');
    ...
    if (!designSystemConfig || !designSystemConfig.enabled) {
      ...
      console.log(chalk.cyan('   ingvar init\n'));
      ...
      const { shouldInit } = await inquirer.prompt([{
        type: 'confirm',
        name: 'shouldInit',
        message: 'Would you like to run ingvar init now?',
        default: true
      }]);
      ...
      if (shouldInit) {
        ...
        console.log(chalk.yellow('\n❌ Spark cancelled. Run "ingvar init" when ready.\n'));
        process.exit(0);
      }
    }
  } catch (error) {
  ...
  try {
    // PHASE 1: Get design system from config (set by ingvar init)
