...
class HuntCommands {
  ...
  static async start(options = {}) {
    ...
    try {
      ...
      if (!config) {
        console.error(chalk.red('❌ No Ingvar Kit configuration found. Run "ingvar team init" first.'));
        return;
      }
      ...
      console.log(chalk.dim(`   3. Track progress with: ingvar hunt status ${hunt.id}\n`));
  ...
  static async analytics(options = {}) {
    try {
      const engine = await AnalyticsEngine.load('Ingvar Kit', '.');
