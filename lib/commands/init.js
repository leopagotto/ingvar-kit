...
async function initCommand(options) {
  ...
  console.log(chalk.yellow('\n🦁 ') + chalk.cyan.bold('Initializing Ingvar Workflow Kit') + chalk.yellow(' 🦁\n'));
...
`;
  ...
  if (selectedAIs.length > 0) {
    ...
    try {
      ...
      if (!isNonInteractive) {
        ...
        console.log(chalk.gray('ingvar can route work to specialized agents based on task type.\n'));
        console.log(chalk.gray(`Recommended for ${projectType} projects:`), chalk.yellow(recommendedAgents.map(a => a.name).join(', ')), '\n');

        const { agentSelection } = await inquirer.prompt([
          {
            type: 'checkbox',
            name: 'agentSelection',
            message: 'Which specialized agents would you like to enable?',
            choices: [
              {
                name: '🎨 Frontend Agent - UI/UX, components, styling, accessibility',
                value: 'frontend',
                checked: recommendedAgents.some(a => a.agent === 'frontend')
              },
              {
                name: '⚙️  Backend Agent - APIs, databases, authentication, security',
                value: 'backend',
                checked: recommendedAgents.some(a => a.agent === 'backend')
              },
              {
                name: '🚀 DevOps Agent - CI/CD, Docker, Kubernetes, monitoring',
                value: 'devops',
                checked: recommendedAgents.some(a => a.agent === 'devops')
              },
              {
                name: '🧪 Testing Agent - Unit/integration/E2E tests, TDD, coverage',
                value: 'testing',
                checked: recommendedAgents.some(a => a.agent === 'testing')
              },
              {
                name: '📚 Documentation Agent - README, API docs, guides, comments',
                value: 'documentation',
                checked: recommendedAgents.some(a => a.agent === 'documentation')
              }
            ]
          }
        ]);

        // Build enabled agents config
        enabledAgents.orchestrator = { enabled: true }; // Always enabled
        agentSelection.forEach(agent => {
          enabledAgents[agent] = { enabled: true };
        });

        if (agentSelection.length > 0) {
          console.log(chalk.green(`\n✓ Enabled agents: orchestrator, ${agentSelection.join(', ')}\n`));
        } else {
          console.log(chalk.gray('\n  Orchestrator-only mode (all tasks routed to single AI)\n'));
        }
      } else {
        // Non-interactive: Enable recommended agents
        console.log(chalk.gray(`  • Agents: Recommended for ${projectType} (${recommendedAgents.length} enabled)`));
        enabledAgents.orchestrator = { enabled: true };
        recommendedAgents.forEach(rec => {
          enabledAgents[rec.agent] = { enabled: true };
        });
      }
      ...
      console.log(chalk.yellow('\n💡 You can generate them later with: ingvar ai sync\n'));
    }
  }
  ...
  // Success message
  console.log(chalk.yellow('\n🦁 ') + chalk.green.bold('Ingvar Workflow Kit initialized successfully!') + chalk.yellow(' 🦁\n'));
  ...
  console.log(chalk.gray('     ingvar issue\n'));
  console.log(chalk.white('  3. Manage AI assistants:'));
  console.log(chalk.gray('     ingvar ai list         # Show configured AIs'));
  console.log(chalk.gray('     ingvar ai add <name>   # Add more AIs'));
  console.log(chalk.gray('     ingvar ai sync         # Regenerate instructions\n'));
  console.log(chalk.white('  4. Check workflow status:'));
  console.log(chalk.gray('     ingvar status\n'));
  console.log(chalk.white('  5. View documentation:'));
  console.log(chalk.gray('     ingvar docs\n'));
