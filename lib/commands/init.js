  ...
  if (!config.org || !config.project) {
    const answers = await inquirer.prompt([
      ...
      },
      {
        type: 'input',
        name: 'project',
        message: 'GitHub project number (or leave empty to skip):',
        when: !config.project
      }
    ]);
...
`;
  ...
  if (config.project && config.org) {
    const projectSpinner = ora('Adding repository to project board...').start();
    try {
      const repoUrl = `https://github.com/${repoInfo}`;
      execSync(
        `gh project item-add ${config.project} --owner ${config.org} --url ${repoUrl}`,
        { stdio: 'ignore' }
      );
      projectSpinner.succeed('Added to project board');
    } catch (error) {
      projectSpinner.warn('Could not add to project board (may already exist)');
    }
  }
