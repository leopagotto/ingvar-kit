...
async function initCommand(options) {
  ...
  console.log(chalk.yellow('\n🦁 ') + chalk.cyan.bold('Initializing Ingvar Kit') + chalk.yellow(' 🦁\n'));
...
`;
      ...
      if (successCount > 0) {
        ...
        if (installIKEAComponents) {
          ...
          try {
            ...
            console.log(chalk.yellow('  You can install them later with: ingvar components install'));
          }
        }

        // Install CWDS components if enabled (NEW - v6.1.0)
        if (installCWDSComponents) {
          const cwdsSpinner = ora('Installing CWDS components...').start();
          try {
            const { CWDSInstaller } = require('../components/cwds-installer');
            const cwdsInstaller = new CWDSInstaller();

            // Auto-select recommended components for non-interactive mode
            ...
            cwdsSpinner.succeed('CWDS components installed');
            console.log(chalk.gray('  → Layout (CWDS & ILOFF)'));
            console.log(chalk.gray('  → Global Header'));
            console.log(chalk.gray('  → App Switcher'));
            console.log(chalk.gray('  → Mobile Navigation'));
            console.log(chalk.gray('  → Navigation Menu'));
            console.log(chalk.gray('  → User Profile'));
            console.log(chalk.gray('  → Example components in src/components/cwds/'));
            console.log(chalk.gray('  → CSS imports in src/styles/cwds.css'));
          } catch (error) {
            cwdsSpinner.fail('Failed to install CWDS components');
            console.log(chalk.yellow('  You can install them later with: ingvar cwds install'));
  ...
  // Success message
  console.log(chalk.yellow('\n🦁 ') + chalk.green.bold('Ingvar Kit initialized successfully!') + chalk.yellow(' 🦁\n'));
