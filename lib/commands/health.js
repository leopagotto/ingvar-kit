...
async function healthCheckCommand() {
  ...
  if (await fs.pathExists(templateDir)) {
    ...
    if (templateCount >= 4) {
      ...
      checks.push({ category: 'Templates', name: 'Issue Templates', status: 'warn', points: 7, detail: `${templateCount} templates`, fix: 'ingvar init to install all templates' });
      score += 7;
    }
  } else {
    checks.push({ category: 'Templates', name: 'Issue Templates', status: 'fail', points: 0, fix: 'ingvar init' });
  }
  ...
  if (!prTemplateFound) {
    checks.push({ category: 'Templates', name: 'PR Template', status: 'warn', points: 0, fix: 'ingvar init to install PR template' });
  }
  ...
  if (await fs.pathExists('.vscode/settings.json')) {
    ...
    checks.push({ category: 'VS Code', name: 'Settings', status: 'warn', points: 0, fix: 'ingvar vscode' });
  }
  ...
  if (await fs.pathExists('.vscode/extensions.json')) {
    ...
    checks.push({ category: 'VS Code', name: 'Extensions', status: 'warn', points: 0, fix: 'ingvar vscode' });
  }
  ...
  if (!copilotFound) {
    checks.push({ category: 'VS Code', name: 'Copilot Instructions', status: 'warn', points: 0, fix: 'ingvar vscode' });
  }
  ...
  if (percentage >= 90) {
    ...
    scoreColor = chalk.yellow;
    grade = 'D';
    message = 'Needs improvement. Run ingvar init to get started. 🔧';
  } else {
    scoreColor = chalk.red;
    grade = 'F';
    message = 'Run ingvar init to set up your workflow. 🚀';
  }
  ...
  console.log(chalk.gray('💡 Run `ingvar welcome` for setup guidance\n'));
}
