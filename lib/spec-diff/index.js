...
class SpecDiffManager {
  ...
  async diff(issueNumber, options = {}) {
    ...
    // Step 3: Compare versions and generate diff
    const changes = await this._compareVersions(versions);

    // Step 4: Display diff
    if (options.timeline) {
      this._displayTimeline(versions);
    } else {
      this._displayDiff(changes, versions);
    }
   ...
   * @private
   */
  async _compareVersions(versions) {
    const changes = [];
    ...
    for (let i = 1; i < versions.length; i++) {
      ...
      const sectionChanges = this._compareSections(prev.sections, curr.sections);
   ...
   * @private
   */
  _compareSections(prevSections, currSections) {
   ...
   * @private
   */
  _displayDiff(changes, versions) {
    if (changes.length === 0) {
      console.log(chalk.yellow('✅ No changes detected between versions\n'));
      return;
    }

    console.log(chalk.bold.cyan(`📝 Spec Changes (${changes.length} edit${changes.length > 1 ? 's' : ''})\n`));
      ...
      for (const sectionChange of change.changes) {
        ...
        if (sectionChange.type === 'list') {
          ...
          if (sectionChange.from) {
            console.log(chalk.red(`    - ${this._truncate(sectionChange.from)}`));
          }
          if (sectionChange.to) {
            console.log(chalk.green(`    + ${this._truncate(sectionChange.to)}`));
          }
        }
        console.log('');
      }
