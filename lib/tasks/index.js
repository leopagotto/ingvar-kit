...
class TaskManager {
    ...
    for (const phase of tasks) {
      ...
      for (const task of phase.tasks) {
        try {
          ...
          issueBody += `_Generated by \`leo tasks create #${parentIssueNumber} --create-issues\`_\n`;
          ...
          const bodyFile = path.join('/tmp', `leo-task-${Date.now()}-${Math.random()}.md`);
  ...
  async _linkChildIssuesToParent(parentIssueNumber, childIssues) {
    try {
      ...
      comment += '_Child issues created by `leo tasks create --create-issues`_\n';

      const commentFile = path.join('/tmp', `leo-child-links-${Date.now()}.md`);
      await fs.writeFile(commentFile, comment, 'utf-8');
