...
const ALL_COMPONENTS = {
  ...
  foundations: [
    { name: 'design-tokens', label: '🎨 Design Tokens', description: 'Core design tokens (colors, spacing, typography)', category: 'Foundations', checked: true },
    { name: 'colours', label: '🌈 Colors', description: 'IKEA color palette', category: 'Foundations', checked: true },
  ...
  // Modals & Overlays (4)
  modals: [
    { name: 'modal-prompt', label: '📦 Modal Prompt', description: 'Prompt modals', category: 'Modals', checked: true },
...
class ComponentInstaller {
  ...
  async installNpmPackages() {
    ...
    const packages = this.selectedComponents.map(name => `@ingka/${name}`);
    ...
    try {
      ...
      console.log(chalk.green(`\n✅ Installed ${packages.length} Ingka packages\n`));
    } catch (error) {
      console.log(chalk.yellow('\n⚠️  Some packages could not be installed from Ingka registry'));
      console.log(chalk.gray('   This might be due to:'));
      console.log(chalk.gray('   - Registry authentication required (IKEA internal only)'));
      console.log(chalk.gray('   - Network issues'));
      console.log(chalk.gray('   - Package not available\n'));
      console.log(chalk.cyan('   Continuing with local template installation...\n'));
    }
  }

  /**
   * Copy component templates from ingvar-kit
   */
  async copyComponentTemplates() {
    console.log(chalk.cyan('📂 Copying component templates...\n'));
    ...
    for (const componentName of this.selectedComponents) {
      const componentDir = path.join(templatesDir, componentName);
    ...
    console.log(chalk.green(`\n✅ Copied ${this.selectedComponents.length} component templates\n`));
  }
  ...
  async generateExports() {
    ...
    const exports = this.selectedComponents
      .map(name => {
...
export * from '@ingka/design-tokens';
export * from '@ingka/colours';
export * from '@ingka/typography';
`;
...
Installed ${this.selectedComponents.length} production-ready IKEA components.
...
${this.selectedComponents.map(name => `- \`${name}\``).join('\n')}

## Documentation

- Official Ingka documentation: Available on IKEA intranet
...
- \`@ingka\` packages: \`https://npm.m2.blue.cdtapps.com\` (IKEA internal)
- Local templates: \`templates/ingka-components/\` (fallback)
...
`;
  ...
  showSuccessMessage() {
    ...
    console.log(chalk.cyan('📁 Components installed to:'), chalk.bold('src/components/ingka/'));
    console.log(chalk.cyan('📦 Packages installed:'), chalk.bold(`${this.selectedComponents.length} @ingka/* packages`));
    console.log();
