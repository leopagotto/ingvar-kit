...
class CostTracker {
  ...
  async loadUsage() {
    try {
      ...
      if (exists) {
        ...
        const lastMonth = data.month; // Format: 'YYYY-MM'
        ...
        // Calculate current daily cost from dailyUsage structure (after reset check)
        const today = new Date().toISOString().split('T')[0];
        const todayUsage = this.usage.dailyUsage?.[today];
        this.usage.dailyCost = todayUsage?.cost || 0;

        // Calculate monthly cost from monthlyUsage structure (after reset check)
        if (this.usage.monthlyUsage) {
          this.usage.monthlyCost = this.usage.monthlyUsage.cost || 0;

          // Also update agents from monthlyUsage
          if (this.usage.monthlyUsage.byAgent) {
            this.usage.agents = this.usage.agents || {};
            for (const [agent, cost] of Object.entries(this.usage.monthlyUsage.byAgent)) {
  ...
  initializeUsage() {
    return {
      ...
      models: {},
      history: [],
      dailyUsage: {},   // Daily usage by date
      monthlyUsage: {   // Monthly usage aggregated
        cost: 0,
        requests: 0,
        byAgent: {},
        byModel: {}
  ...
  /**
   * Get daily usage stats for today
  ...
  getDailyUsage() {
    ...
    if (this.usage.dailyUsage?.[today]) {
      return this.usage.dailyUsage[today];
    }

    return {
      cost: this.usage.dailyCost || 0,
      requests: 0,
      byAgent: {},
      byModel: {}
    };
