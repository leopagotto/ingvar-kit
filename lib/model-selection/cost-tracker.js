...
class CostTracker {
  ...
  loadUsage() {
    try {
      if (fs.existsSync(this.usageFile)) {
        const data = fs.readJsonSync(this.usageFile);
        
        // Check if we need to reset (new month)
        const lastMonth = new Date(data.lastReset || 0).getMonth();
        const currentMonth = new Date().getMonth();
        
        if (lastMonth !== currentMonth) {
          return this.resetMonthlyUsage(data);
        }
  ...
  async saveUsage() {
    try {
      ...
      await fs.writeJson(this.usageFile, this.usage, { spaces: 2 });
    } catch (error) {
  ...
  calculateCost(model, usage) {
    ...
    const modelPricing = pricing[model] || { input: 1.00, output: 2.00 };
