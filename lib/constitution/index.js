...
class ConstitutionManager {
  constructor(projectRoot = process.cwd()) {
    ...
    this.configPath = path.join(projectRoot, '.ingvarrc.json');
    this.constitutionDocPath = path.join(projectRoot, 'docs', 'CONSTITUTION.md');
  }

  /**
   * Initialize constitutional principles for a project
   * Creates both .ingvarrc.json config and docs/CONSTITUTION.md reference
   *
  ...
  async init(options = {}) {
    ...
    console.log(`   ${chalk.bold('Config:')} .ingvarrc.json`);
  ...
  /**
   * Save constitution to .ingvarrc.json configuration
   *
  ...
  async saveToConfig(constitution) {
    ...
    try {
      ...
      console.log(chalk.yellow('No existing .ingvarrc.json found, creating new one...'));
    }
