...
class RoleManager {
  ...
  /**
   * Get role by ID
   * @param {string} roleId - requirements|spec|implementation|testing
  ...
  static findRoleByKeyword(text) {
    ...
    for (const role of Object.values(ROLES)) {
      ...
      if (matchCount > 0) {
        matches.push({ role, matchCount });
      }
    }

    if (matches.length === 0) return null;

    // Return role with most matches, sorted by sequence order if tie
    matches.sort((a, b) => {
  ...
  /**
   * Check if transition is valid (roleA to roleB must be sequential)
   * @param {string} roleAId
   * @param {string} roleBId
   * @returns {boolean}
   */
  static isValidTransition(roleAId, roleBId) {
    ...
    return nextRole ? nextRole.id === roleBId : false;
  }
}

/**
 * Export for use throughout LionPack
 */
module.exports = {
  ROLES,
  RoleManager
