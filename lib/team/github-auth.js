 ...
 * Stores token securely in .lionpack directory
 *
 * @class GitHubAuth
 */
class GitHubAuth {
  ...
  constructor(token = null) {
    this.token = token;
    this.tokenFile = path.join(process.cwd(), '.lionpack', 'github-token');
    this.rateLimit = {
      remaining: 5000,
   ...
   * Reads token from .lionpack/github-token file
   *
   * @static
   ...
   */
  static loadToken() {
    const tokenFile = path.join(process.cwd(), '.lionpack', 'github-token');

    if (!fs.existsSync(tokenFile)) {
      throw new Error('GitHub token not found. Run: leo github setup');
    }
   ...
   * Stores token in .lionpack/github-token file
   * Creates .lionpack directory if it doesn't exist
   *
   * @static
  ...
  static saveToken(token) {
    ...
    const lionpackDir = path.join(process.cwd(), '.lionpack');
    const tokenFile = path.join(lionpackDir, 'github-token');

    try {
      // Create .lionpack directory if it doesn't exist
