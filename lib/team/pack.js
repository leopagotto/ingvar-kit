...
class TeamPack {
  ...
  /**
   * Load pack from .lionpack.json
   */
  static async load(projectPath = '.') {
    const configPath = path.join(projectPath, '.lionpack.json');
    ...
    try {
      ...
      if (error.code === 'ENOENT') {
        throw new Error(
          'Pack not initialized. Run "lionpack team init" first.'
        );
      }
      throw error;
    }
  ...
  /**
   * Verify member exists on GitHub (stub for now)
   */
  static async verifyGitHubUser(username) {
    // TODO: Call GitHub API to verify user exists
