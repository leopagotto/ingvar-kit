 ...
 * LionPack API Server
 *
 * REST API and WebSocket server for LionPack dashboard
...
class APIServer extends EventEmitter {
  ...
  _handleConnection(socket) {
    ...
    socket.emit('connected', {
      message: 'Connected to LionPack server',
      clientId: socket.id,
      timestamp: new Date().toISOString()
    });
  ...
  async _getHunts(req, res) {
    try {
      ...
      const hunts = this.tracker.hunts.map(h => ({
        id: h.id,
  ...
  async _getHuntDetail(req, res) {
    try {
      ...
      res.json({
        ...
        slackChannel: hunt.slackChannel
      });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
