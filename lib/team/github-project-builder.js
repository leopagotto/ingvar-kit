/**
 * Ingvar Kit GitHub Project Board Builder
...
class GitHubProjectBuilder {
  ...
  getProjectConfig() {
    return {
      name: `🦁 Ingvar Kit Hunt ${this._getPackName()}`,
  ...
  generateGitHubWorkflow() {
    const workflow = {
      name: 'Ingvar Kit Hunt Automation',
      on: {
        issues: {
  ...
  generateSetupScript(members) {
    ...
    let script = `#!/bin/bash
# Ingvar Kit Project Board Setup Script
# Generated for: ${this.repoOwner}/${this.repoName}

set -e

echo "🦁 Setting up Ingvar Kit project board..."
echo "Team Size: ${this.teamSize}"
echo ""

# Create project
PROJECT_NAME="🦁 Ingvar Kit Hunt - ${this._getPackName()}"
...
`;
  ...
  generateBoardDocumentation(members) {
    ...
    let doc = `# 🦁 Ingvar Kit Project Board Guide\n\n`;
  ...
  _getProjectDescription() {
    ...
    return descriptions[this.teamSize] || 'Ingvar Kit team workflow';
  }
}
