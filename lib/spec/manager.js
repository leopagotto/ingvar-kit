...
class SpecificationManager {
  ...
  async init(featureName) {
    try {
      ...
      console.log(`  ingvar spec constitution       # Define project principles`);
      console.log(`  ingvar spec specify            # Write specification`);
      console.log(`  ingvar spec plan               # Plan implementation`);
      console.log(`  ingvar spec tasks              # Generate tasks\n`);
  ...
  async createConstitution(content) {
    try {
      if (!this.featureDir) {
        throw new Error('Feature not initialized. Run: ingvar spec init <name>');
      }
      ...
      console.log(`  ingvar spec specify\n`);
...
Run: \`ingvar spec implement\` to generate code for these tasks
`;
...
Run: \`ingvar spec tasks\` to auto-generate tasks from your spec.
`;
  }
}
