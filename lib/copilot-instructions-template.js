// Enhanced Copilot Instructions Template for Ingvar Workflow Kit
...
> - ❌ **NEVER use \\\`ingvar issue\\\` command** - it opens interactive CLI that forces manual input
...
**❌ NEVER DO THIS:**
\\\`\\\`\\\`bash
ingvar issue  # Opens interactive CLI - forces user to fill form manually - FORBIDDEN!
\\\`\\\`\\\`
...
- Use \`gh issue create\` command (not interactive \`ingvar issue\`)
...
- Use interactive \`ingvar issue\` (use \`gh issue create\` instead)
...
**🦁 Ingvar Workflow Kit - Spec-Driven Development Made Simple 🦁**
